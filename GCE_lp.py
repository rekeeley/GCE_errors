...
plt.plot(bin_center,(k-background)/exposure,label = 'Residual Number Flux')
plt.xscale('log')
plt.legend(loc='best')
plt.savefig('plots/log_parabola_zoa/'+file_path+'/residual_flux.png')
plt.clf()
...
#Eb = np.array([3.])
Eb = 3.
alpha = np.linspace(2.0,3.0,30)
beta = np.linspace(0.4,0.7,40)
...
#print GC_SM_mu /GC_dist_mu**2./4./np.pi
...
binned_spectra = GCE_calcs.calculations.get_spec_log_parab_zoa(gfpsm,SM_GC,alpha,beta,Eb,emax_GCE,emin_GCE)
...
plt.savefig('plots/log_parabola_zoa/'+file_path+'/norm.png')
plt.clf()
...
plt.savefig('plots/log_parabola_zoa/'+file_path+'/residuals_GCE.pdf')
plt.clf()

np.savetxt('plots/log_parabola_zoa/'+file_path+'/residuals_GCE.txt', (bin_center, k[0,0,0,0,:], background[0,0,0,0,:], mu_GCE[max_index_GCE[0],max_index_GCE[1],max_index_GCE[2],max_index_GCE[3],:]))
...
plt.savefig('plots/log_parabola_zoa/'+file_path+'/GCE_gfpsm_beta_contours.png')
plt.clf()
...
plt.ylabel(r'Gamma ray rate per stellar mass [s$^{-1}$ M$_\odot$]')
plt.yscale('log')
plt.savefig('plots/log_parabola_zoa/'+file_path+'/GCE_gfpsm_alpha_contours.png')
plt.clf()
...
plt.savefig('plots/log_parabola_zoa/'+file_path+'/GCE_alpha_beta_contours.png')
plt.clf()
...
plt.ylabel('Scaled probability')
plt.legend(loc='upper right',frameon=False,fontsize=22)
plt.savefig('plots/log_parabola_zoa/'+file_path+'/GCE_norm_posterior.pdf')
plt.clf()
...
for i in range(len(like_name)):
    ...
    nflux_dwarf = GCE_calcs.calculations.get_spec_log_parab_zoa(gfpsm,norm_dwarf,alpha,beta,Eb,emax_dwarf,emin_dwarf)
    ...
    plt.xlabel('Norm')
    plt.ylabel('Scaled Probability')
    plt.savefig('plots/log_parabola_zoa/'+file_path+'/'+name+'_norm.png')
    plt.clf()
    ...
    plt.savefig('plots/log_parabola_zoa/'+file_path+'/'+name+'_dwarf_gfpsm_beta_contours.png')
    plt.clf()
    ...
    plt.ylabel(r'Gamma ray rate per stellar mass [s$^{-1}$ M$_\odot$]')
    plt.yscale('log')
    plt.savefig('plots/log_parabola_zoa/'+file_path+'/'+name+'_dwarf_gfpsm_alpha_contours.png')
    plt.clf()
    ...
    plt.ylabel('Scaled Probability')
    plt.ylim(0,1.1)
    plt.savefig('plots/log_parabola_zoa/'+file_path+'/'+name+'_gfpsm_posterior.png')
    plt.clf()
...
plt.savefig('plots/log_parabola_zoa/'+file_path+'/dwarf_alpha_beta_contours.png')
plt.clf()
...
plt.ylim(0,1.1)
plt.legend(loc = 'upper right', frameon=False, fontsize=22)
plt.savefig('plots/log_parabola_zoa/'+file_path+'/gfpsm_posteriors.pdf')
plt.clf()
...
plt.xlabel('beta')
plt.ylabel('alpha')
plt.savefig('plots/log_parabola_zoa/'+file_path+'/combo_alpha_beta_contours.png')
plt.clf()
...
np.savetxt('plots/log_parabola_zoa/'+file_path+'/log_parab_evidences.txt',np.array([evidence_GCE, evidence_dwarf, evidence_dwarf*evidence_GCE, evidence_combo, evidence_dwarf*evidence_GCE/evidence_combo, like_GCE_4d.max()]) )
