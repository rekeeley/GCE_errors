...
def main():
    ...
    parser.add_argument('--model', type=int, default=0,
        help='background model used to generate the data used for the GCE: kwa=0, glliem=1, gal2yr=2, MIT=3')
...
##### ANALYSIS #####
    if model_name=='MIT':
        binned_spectra = np.loadtxt('spectra/binned_0414/'+channel_name+'/binned_spectra_'+channel_name+'_MIT.txt')[:,trunc:]
    else:
        binned_spectra = np.loadtxt('spectra/binned_0414/'+channel_name+'/binned_spectra_'+channel_name+'_GCE.txt')[:,trunc:]#difference due to different energy bins

    #This is a binned dPhi/dE
...
########################
    ...
    plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/fluxes.png')
    plt.clf()
    ...
    plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/GCE_contours.pdf')
    plt.clf()

    np.savetxt('plots/WIMP/'+channel_name+'_'+model_name+'_profile/sigma_mass_posterior.txt', (-np.log(GCE_like_2d) + np.log(GCE_like_2d.max())))
...
###################
    ...
    for i in range(len(like_name)):
        ...
        plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/'+name+'_contours.png')
        plt.clf()
    ...
    plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/dwarf_contours.pdf')
    plt.clf()

    np.savetxt('plots/WIMP/'+channel_name+'_'+model_name+'_profile/dwarf_contours.txt',-np.log(like_dwarf_2d/like_dwarf_2d.max()))
    ...
    plt.ylim(0,1.1)
    plt.legend(loc='upper right', frameon=False, fontsize=22)
    plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/cross_section_posteriors.pdf')
    plt.clf()
...
####################
    ...
    plt.xlabel('Mass [GeV]')
    plt.ylabel('Cross Section [cm^3 sec^-1]')
    plt.savefig('plots/WIMP/'+channel_name+'_'+model_name+'_profile/combo_contours.png')
    plt.clf()

    np.savetxt('plots/WIMP/'+channel_name+'_'+model_name+'_profile/cross_section_posteriors.txt',(sigma,like_dwarf_1d/like_dwarf_1d.max(),like_GCE_1d/like_GCE_1d.max()))
    ...
    np.savetxt('plots/WIMP/'+channel_name+'_'+model_name+'_profile/evidences.txt',(evidence_GCE, evidence_dwarf, evidence_dwarf*evidence_GCE, evidence_combo, evidence_dwarf*evidence_GCE/evidence_combo, GCE_like_3d.max()) )
