...
levels = [0,1.2,3.3,6.35,15]# \Delt4 log-like  = (1,3,6)
...
plt.text(6,5e-26,'Thermal Relic Cross Section',color='k',fontsize=10)
plt.text(6, 3.5e-26,'(Steigman et al. 2012)',color='k',fontsize=10)
plt.text(6, 1.5e-27,r'$b\bar{b}$',fontsize=16)
plt.xscale('log')
plt.yscale('log')
plt.ylim(1e-27,1e-23)
plt.xlim(5,1e2)
plt.xlabel('Mass (GeV)',fontsize=28)
plt.ylabel(r'$\langle \sigma v\rangle$ (cm$^3$ s$^{-1}$)',fontsize=28)
plt.legend(loc='upper left', prop = {'size':14})
plt.gcf().subplots_adjust(bottom=0.19)
plt.gcf().subplots_adjust(left=0.2)
plt.gcf().subplots_adjust(right=.94)
plt.savefig('write-up/plot_digi.pdf')#plotting the data
plt.clf()
