...
model_name = ['kwa','glliem','gal2yr','MIT'][model]

trunc=0

loaded = np.load('data/background/GC_'+model_name+'.npz')
emin_GCE = loaded['emax'][trunc:,0]/1000.
emax_GCE = loaded['emin'][trunc:,0]/1000. #convert form MeV to GeV; these got switched in creating the npz file
...
flux_max = np.zeros(len(emin_GCE))
...
#the total normalization of the spectra is the product of the stellar mass of the region times the the gamma-ray flux per stellar mass.  The first changes between the regions and the second is the same.  This is the zeroth order ansatz
...
log_like_GCE_5d = GCE_calcs.analysis.GCE_delta_log_like_msp(nflux_GCE,model_name,trunc)
...
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/norm.png')
plt.clf()

plt.plot(bin_center, flux_max, label='Best fit flux')
plt.plot(bin_center, nflux_GCE[max_index_GCE[0],max_index_GCE[1],max_index_GCE[2],max_index_GCE[3],:], label='best fit model')
plt.xscale('log')
plt.yscale('log')
plt.ylim(1e-10,1e-7)
plt.legend(loc='upper right', frameon=False, fontsize=22)
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/fluxes.png')
plt.clf()
...
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/GCE_gfpsm_beta_contours.png')
plt.clf()
...
plt.ylabel(r'Gamma ray rate per stellar mass [s$^{-1}$ M$_\odot$]')
plt.yscale('log')
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/GCE_gfpsm_alpha_contours.png')
plt.clf()
...
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/GCE_alpha_beta_contours.png')
plt.clf()
...
plt.ylabel('Scaled probability')
plt.legend(loc='upper right',frameon=False,fontsize=22)
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/GCE_norm_posterior.pdf')
plt.clf()
...
for i in range(len(like_name)):
    ...
    plt.xlabel('Norm')
    plt.ylabel('Scaled Probability')
    plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/'+name+'_norm.png')
    plt.clf()
    ...
    plt.yscale('log')
    plt.ylabel(r'Gamma ray rate per stellar mass [s$^{-1}$ M$_\odot$]')
    plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/'+name+'_dwarf_gfpsm_beta_contours.png')
    plt.clf()
    ...
    plt.ylabel(r'Gamma ray rate per stellar mass [s$^{-1}$ M$_\odot$]')
    plt.yscale('log')
    plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/'+name+'_dwarf_gfpsm_alpha_contours.png')
    plt.clf()
    ...
    plt.ylabel('Scaled Probability')
    plt.ylim(0,1.1)
    plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/'+name+'_gfpsm_posterior.png')
    plt.clf()
...
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/dwarf_alpha_beta_contours.png')
plt.clf()
...
plt.ylim(0,1.1)
plt.legend(loc = 'upper right', frameon=False, fontsize=22)
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/gfpsm_posteriors.pdf')
plt.clf()
...
plt.xlabel('beta')
plt.ylabel('alpha')
plt.savefig('plots/'+channel_name+'/'+model_name+'_ll/combo_alpha_beta_contours.png')
plt.clf()
...
np.savetxt('plots/'+channel_name+'/'+model_name+'_ll/log_parab_evidences.txt',np.array([evidence_GCE, evidence_dwarf, evidence_dwarf*evidence_GCE, evidence_combo, evidence_dwarf*evidence_GCE/evidence_combo, like_GCE_4d.max()]) )
