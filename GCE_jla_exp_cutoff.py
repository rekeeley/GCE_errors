...
p_prior_norm = np.trapz(np.ones(n_p),x = p)
...
plt.legend(loc='best')
plt.savefig('plots/exp_cutoff/'+file_name+'test_residuals_GCE.png')
plt.clf()

like_GCE_2d_N0_p = np.trapz(np.exp(log_like_GCE_3d), x=gamma, axis=1)
like_GCE_2d_N0_gamma = np.trapz(np.exp(log_like_GCE_3d), x=p, axis=2)
...
plt.savefig('plots/exp_cutoff/'+file_name+'_GCE_N0_p_contours.png')
plt.clf()
...
plt.ylabel('Normalization')
plt.yscale('log')
plt.savefig('plots/exp_cutoff/'+file_name+'_GCE_N0_gamma_contours.png')
plt.clf()

GCE_like_2d = np.trapz(np.exp(log_like_GCE_3d), x=np.log(N0_GCE), axis=0)/N0_prior_norm

CS = plt.contour(p,gamma,-np.log(GCE_like_2d/GCE_like_2d.max()),levels)
plt.clabel(CS, inline=1, fontsize=10)
plt.xlabel('p')
plt.ylabel('gamma')
plt.savefig('plots/exp_cutoff/'+file_name+'_GCE_contours.png')
plt.clf()



GCE_like_1d = np.trapz(np.trapz(np.exp(log_like_GCE_3d), x=gamma, axis=1), x=p, axis=1)

plt.plot(N0_GCE,GCE_like_1d/GCE_like_1d.max())
plt.xlabel('Normalization')
plt.xscale('log')
plt.ylabel('Scaled probability')
plt.savefig('plots/exp_cutoff/'+file_name+'_GCE_norm_posterior.png')
plt.clf()


evidence_GCE = np.trapz(np.trapz(GCE_like_2d, x=gamma, axis=0), x=p, axis=0)/gamma_prior_norm/p_prior_norm
...
for name in like_name:
    ...
    like_dwarf_2d_N0_gamma = np.trapz(like_dwarf_3d,x=p,axis=2)
    CS = plt.contour(p,N0_dwarf,-np.log(like_dwarf_2d_N0_p/like_dwarf_2d_N0_p.max()),levels)
    plt.clabel(CS, inline=1, fontsize=10)
    plt.xlabel('p')
    plt.yscale('log')
    plt.ylabel('Normalization')
    plt.savefig('plots/exp_cutoff/'+file_name+'_'+name+'_N0_p_contours.png')
    plt.clf()
    ...
    plt.ylabel('Normalization')
    plt.yscale('log')
    plt.savefig('plots/exp_cutoff/'+file_name+'_'+name+'_N0_gamma_contours.png')
    plt.clf()
    like_dwarf_2d *= np.trapz(like_dwarf_3d, x=np.log(N0_dwarf), axis=0)/N0_dwarf_normalization
    dwarf_norm_posterior = np.trapz(np.trapz(like_dwarf_3d, x=gamma, axis=1), x=p, axis=1)
    plt.plot(N0_dwarf,dwarf_norm_posterior/dwarf_norm_posterior.max())
    plt.xlabel('Normalization')
    plt.xscale('log')
    plt.ylabel('Scaled Probability')
    plt.ylim(0,1.1)
    plt.savefig('plots/exp_cutoff/'+file_name+'_'+name+'_norm_posterior.png')
    plt.clf()

CS = plt.contour(p,gamma,-np.log(like_dwarf_2d/like_dwarf_2d.max()),levels)
plt.clabel(CS, inline=1, fontsize=10)
plt.xlabel('p')
plt.ylabel('gamma')
plt.savefig('plots/exp_cutoff/'+file_name+'_dwarf_contours.png')
plt.clf()


evidence_dwarf =  np.trapz(np.trapz(like_dwarf_2d ,x = gamma,axis=0),x=p,axis=0)/gamma_prior_norm/p_prior_norm
...
CS = plt.contour(p,gamma,-np.log(combo_like_2d/combo_like_2d.max()),levels)
plt.clabel(CS, inline=1, fontsize=10)
plt.xlabel('p')
plt.ylabel('gamma')
plt.savefig('plots/exp_cutoff/'+file_name+'_combo_contours.png')
plt.clf()


evidence_combo = np.trapz(np.trapz(combo_like_2d, x=gamma, axis=0), x=p, axis=0)/gamma_prior_norm/p_prior_norm
